...
class Entete extends React.Component {
      ...
      this.state.media.rightHolders.forEach(r => {
        if (!this.avatars[r.id]) {
          ...
          if (props.rightHolders[r.id]) {
            ...
            avatar = `${config.IMAGE_SRV_URL}${
              props.rightHolders[r.id].avatarImage
              }`
          } else {
            uuid = " "
            nom = " "
  ...
  renderAvatars() {
    ...
    if (this.avatars.length >= maxDisplayedAvatars) {
      let autres = ""
      this.avatars
        .slice(maxDisplayedAvatars, this.avatars.length)
        .forEach(e => {
          autres =
            autres +
  ...
  getMedia() {
    axios
      .get(`${config.API_URL}media/${this.state.media.mediaId}`)
      .then(res => {
  ...
  render() {
    ...
    if (
      ...
      this.state.media.files.cover &&
      this.state.media.files.cover.files &&
      this.state.media.files.cover.files.length > 0
    ) {
        ...
        <div className={"ui container flex"}>
          ...
          <div className={"song-info"}>
            {this.state.editerTitre && (
              <div className="ui input">
                ...
                <i
                  ...
                  className="save alternate icon grey big edit title"
                ></i>
              </div>
            )}
            {!this.state.editerTitre && (
            ...
            <div className={"other-info"}>
              ...
              <span>
								{t("oeuvre.creePar")}{" "}
								{this.props.rightHolders[this.state.media.creator].artistName}
							</span>{" "} */}
              ...
              {i18n.language &&
                moment(
                  new Date(
                    parseInt(
                      this.state.media.modificationDate
                        ? this.state.media.modificationDate
                        : this.state.media.creationDate,
