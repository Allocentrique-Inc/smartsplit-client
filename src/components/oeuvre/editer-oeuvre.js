...
class EditerOeuvre extends Component {
	...
	componentWillMount() {
		if (this.state.jeton) {
			...
			axios.get(`${config.API_URL}media/${this.state.mediaId}`).then(_m => {
	...
	getMedia(admin, response = false) {
		if (this.state.mediaId) {
			axios.get(`${config.API_URL}media/${this.state.mediaId}`).then(res => {
				if (res.data) {
					let media = res.data
	...
	chargement(admin = false) {
		if (Identite.usager) {
			this.setState({ user: Identite.usager }, () =>
				this.getMedia(admin, Identite.usager)
			)
		} else {
			if (!admin) {
	...
	renduPage() {
		...
		switch (this.props.pageNo) {
			...
			case "5":
				page = (
					<Wizard.Page>
						<PageInformationsGenerales pochette={this.props.pochette} />
					</Wizard.Page>
				)
				break
			case "6":
	...
	render() {
		...
		if ((this.state.user || this.state.jeton) && this.state.media) {
			...
			return <Connexion parent={this} isOpen={this.state.Connexion} />
		}
	}
}
