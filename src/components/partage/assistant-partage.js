...
class AssistantPartage extends Component {
	constructor(props) {
		...
		this.enregistrerEtAllerAuSommaire = this.enregistrerEtAllerAuSommaire.bind(
			this
		)
	...
	componentWillMount() {
		if (Identite.usager) {
			...
			if (this.state.mediaId) {
				...
				axios
					.get(
						`${config.API_URL}proposal/derniere-proposition/${this.state.mediaId}`
					)
					.then(res => {
				...
				axios
					.get(`${config.API_URL}proposal/${this.state.uuid}`)
					.then(res => {
	...
	validerSommeDroits(values) {
		...
		if (
			parseInt(droitAuteur) > 0 &&
			parseInt(Math.round(droitAuteur)) !== 100
		) {
			toast.error(t("validation.partage.droitAuteur", { somme: droitAuteur }))
			valide = false
		}
		if (
			parseInt(droitInterpretation) > 0 &&
			parseInt(Math.round(droitInterpretation)) !== 100
		) {
			toast.error(
				t("validation.partage.droitInterpretation", {
					somme: droitInterpretation
				})
			)
			valide = false
		}
		if (
			parseInt(droitEnregistrement) > 0 &&
			parseInt(Math.round(droitEnregistrement)) !== 100
		) {
			toast.error(
				t("validation.partage.droitEnregistrement", {
					somme: droitEnregistrement
				})
			)
			valide = false
		}
	...
	validerDroitsNonNegatifs(parts, type) {
		...
		if (!valide) {
			toast.error(
				this.props.t("validation.partage.droitNegatif", { type: type })
			)
		}
		return valide
	}
	...
	render() {
		...
		if (this.state.media && this.state.ayantsDroit) {
			...
			if (this.state.proposition) {
				if (
					this.state.proposition.etat !== "BROUILLON" &&
					this.state.proposition.etat !== "PRET" &&
				...
				function creerAd(elem) {
					if (that.state.ayantsDroit) {
						return {
							nom: elem.rightHolder.name,
							pourcent: 0.0,
							ayantDroit: that.state.ayantsDroit[elem.rightHolder.rightHolderId]
						}
					}
				}
				// Droit d'auteur
				_rS.workCopyrightSplit.music.forEach(elem => {
					// Musique
					if (!_droit.auteur[elem.rightHolder.rightHolderId]) {
						_droit.auteur[elem.rightHolder.rightHolderId] = creerAd(elem)
					}
					_droit.auteur[
						elem.rightHolder.rightHolderId
					].pourcentMusique = parseFloat(elem.splitPct)
					_droit.auteur[elem.rightHolder.rightHolderId].pourcent =
						parseFloat(elem.splitPct) +
						_droit.auteur[elem.rightHolder.rightHolderId].pourcent
					_droit.auteur[elem.rightHolder.rightHolderId].arrangeur = elem
						.contributorRole[ROLES.ARRANGEUR]
						? true
						: false
					_droit.auteur[elem.rightHolder.rightHolderId].compositeur = elem
						.contributorRole[ROLES.COMPOSITEUR]
						? true
						: false
					_droit.auteur[elem.rightHolder.rightHolderId].color =
						elem.rightHolder.color
				})
				_rS.workCopyrightSplit.lyrics.forEach(elem => {
					// Paroles
					if (!_droit.auteur[elem.rightHolder.rightHolderId]) {
						_droit.auteur[elem.rightHolder.rightHolderId] = creerAd(elem)
					}
					_droit.auteur[
						elem.rightHolder.rightHolderId
					].pourcentParoles = parseFloat(elem.splitPct)
					_droit.auteur[elem.rightHolder.rightHolderId].pourcent =
						parseFloat(elem.splitPct) +
						_droit.auteur[elem.rightHolder.rightHolderId].pourcent
					_droit.auteur[elem.rightHolder.rightHolderId].auteur = elem
						.contributorRole[ROLES.AUTEUR]
						? true
						: false
					_droit.auteur[elem.rightHolder.rightHolderId].color =
						elem.rightHolder.color
				...
				_rS.performanceNeighboringRightSplit.principal.forEach(elem => {
					// Principal
					if (!_droit.interpretation[elem.rightHolder.rightHolderId]) {
						_droit.interpretation[elem.rightHolder.rightHolderId] = creerAd(
							elem
						)
					}
					_droit.interpretation[elem.rightHolder.rightHolderId].pourcent =
						parseFloat(elem.splitPct) +
						_droit.interpretation[elem.rightHolder.rightHolderId].pourcent
					_droit.interpretation[elem.rightHolder.rightHolderId].principal = true
					_droit.interpretation[elem.rightHolder.rightHolderId].chanteur = elem
						.contributorRole[ROLES.CHANTEUR]
						? true
						: false
					_droit.interpretation[elem.rightHolder.rightHolderId].musicien = elem
						.contributorRole[ROLES.MUSICIEN]
						? true
						: false
					_droit.interpretation[elem.rightHolder.rightHolderId].color =
						elem.rightHolder.color
				})
				_rS.performanceNeighboringRightSplit.accompaniment.forEach(elem => {
					// Accompagnement
					if (!_droit.interpretation[elem.rightHolder.rightHolderId]) {
						_droit.interpretation[elem.rightHolder.rightHolderId] = creerAd(
							elem
				...
				_rS.masterNeighboringRightSplit.split.forEach(elem => {
					// Split
					if (!_droit.enregistrement[elem.rightHolder.rightHolderId]) {
						_droit.enregistrement[elem.rightHolder.rightHolderId] = creerAd(
							elem
						)
					}
					_droit.enregistrement[
						elem.rightHolder.rightHolderId
					].pourcent = parseFloat(elem.splitPct)
					_droit.enregistrement[elem.rightHolder.rightHolderId].studio = elem
						.contributorRole[ROLES.STUDIO]
						? true
						: false
					_droit.enregistrement[
						elem.rightHolder.rightHolderId
					].producteur = elem.contributorRole[ROLES.PRODUCTEUR] ? true : false
					_droit.enregistrement[
						elem.rightHolder.rightHolderId
					].realisateur = elem.contributorRole[ROLES.REALISATEUR] ? true : false
					_droit.enregistrement[elem.rightHolder.rightHolderId].graphiste = elem
						.contributorRole[ROLES.GRAPHISTE]
						? true
						: false
					_droit.enregistrement[elem.rightHolder.rightHolderId].color =
						elem.rightHolder.color
				})

				// Construction des valeurs initiales
				Object.keys(_droit.auteur).forEach(elem => {
					valeursInitiales.droitAuteur.push(_droit.auteur[elem])
				})
				Object.keys(_droit.interpretation).forEach(elem => {
					valeursInitiales.droitInterpretation.push(_droit.interpretation[elem])
				})
				Object.keys(_droit.enregistrement).forEach(elem => {
					valeursInitiales.droitEnregistrement.push(_droit.enregistrement[elem])
				})
			}
				...
				<>
					<div className="ui grid" style={{ padding: "10px" }}>
						{lectureSeule && (
							<script>
								setTimeout(()=>
								{toast.info(t("flot.split.partage.lecture-seule"))})
							</script>
						)}
						<div className="ui row">
							...
							<div className="ui twelve wide column">
								<Wizard
									...
									buttonLabels={{
										plusTard: t("navigation.plus-tard"),
										previous: t("navigation.retour"),
										next: t("navigation.suivant"),
										submit: t("navigation.envoi")
									}}
									debug={false}
									onPageChanged={index =>
										this.setState({ currentWizardPage: index })
									}
									onSubmit={(values, actions) => {
										...
										if (!lectureSeule) {
											...
											valide =
												valide &&
												this.validerDroitsNonNegatifs(
													values.droitInterpretation,
													t("validation.interpretation")
												)
											valide =
												valide &&
												this.validerDroitsNonNegatifs(
													values.droitEnregistrement,
													t("validation.enregistrement")
												)
								...
								>
									<Wizard.Page>
										<PageAssistantPartageDroitAuteur
											...
											enregistrerEtAllerAuSommaire={
												this.enregistrerEtAllerAuSommaire
											}
					...
					<Modal
						open={this.state.modaleFin}
						onClose={() => this.modaleFin(false)}
					>
						...
						<div className="modal-content">
							...
							<h4 className={"h4-style"}>{t("flot.fin.maintenantPartage")}</h4>
							{this.props.i18n.language.substring(0, 2) === "en" && (
								<p className={"description"}>
									Hourray, your successfully created a share proposal.{" "}
									<em>Click</em> on the button below to <em>review</em> it's
									content and to <em>send by email</em> to your collaborators.
								</p>
							)}
							{this.props.i18n.language.substring(0, 2) !== "en" && (
								<p className={"description"}>
									Bravo, tu as créé une proposition de partage de droits avec
									succès ! <em>Clique</em> sur le bouton ci-dessous afin de{" "}
									<em>revoir</em> et <em>envoyer par courriel</em> la
									proposition à tes collaborateurs.
								</p>
							)}
						</div>
