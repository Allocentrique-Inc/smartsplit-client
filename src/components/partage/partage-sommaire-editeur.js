...
class PartageSommaireEditeur extends Component {
			...
			this.setState({ beneficiaire: beneficiaire }, () => {
				...
				parts.lyrics.forEach((elem, idx) => {
					if (!_rH[elem.rightHolder.rightHolderId]) {
						_rH[elem.rightHolder.rightHolderId] = {
							nom: undefined,
							pourcent: 0
						}
					}
					...
					_rH[elem.rightHolder.rightHolderId].pourcent =
						parseFloat(_rH[elem.rightHolder.rightHolderId].pourcent) +
				...
				Object.keys(_rH).forEach(elem => {
					if (elem === this.state.part.rightHolderId) {
						...
						donnees.push({
							ayantDroit: this.state.donateur,
							color: _rH[elem].color,
							nom: _rH[elem].nom,
							pourcent: parseFloat(
								(_rH[elem].pourcent * this.state.part.rightHolderPct) / 100
							)
						})
						donnees.push({
							ayantDroit: this.state.beneficiaire,
							color: "#bacada",
							nom: this.state.beneficiaire.artistName
								? this.state.beneficiaire.artistName
								: `${this.state.beneficiaire.firstName} ${this.state.beneficiaire.lastName}`,
							pourcent: parseFloat(
								(this.state.part.shareePct * _rH[elem].pourcent) / 100
							)
						})
					} else {
						// on pousse l'ayant-droit
						donnees.push({
							ayantDroit: this.state.ayantsDroit[elem],
							color: _rH[elem].color,
							nom: _rH[elem].nom,
							pourcent: parseFloat(_rH[elem].pourcent)
						})
					}
				})
	...
	estVoteFinal() {
		...
		return (
			this.state.part.etat === "ACCEPTE" || this.state.part.etat === "REFUSE"
		)
	}
	...
	envoi() {
		...
		axios
			.post(`${config.API_URL}splitShare/tiers/voter`, body)
			.then(res => {
	...
	Connexion(ouvrir = true) {
		this.setState({ Connexion: ouvrir })
	}
	...
	rafraichirDonnees() {
		if (this.state.rafraichir) {
			axios
				.get(
					`${config.API_URL}splitShare/${this.state.proposition.uuid}/${this.state.user.username}`
				)
				.then(res => {
	...
	render() {
		...
		if (this.state.beneficiaire && this.state.donateur) {
			let visualisation = (
				<Beignet
					type="workCopyrightSplit"
					uuid={`auteur--beignet__${this.state.idx}`}
				...
				<>
					<div className="ui grid">
						<div className="ui row" style={{ minHeight: "38rem" }}>
							<div className="ui eight wide column">
								<div className="ui grid">
									...
									<div className="ui row" style={{ padding: "1rem" }}>
										...
										>
											<img
												style={{ margin: "0.5rem 0px 0px 1rem" }}
												alt=""
												className="ui spaced avatar image"
												src={`${config.IMAGE_SRV_URL}${this.state.beneficiaire.avatarImage}`}
											/>
										</div>
										<div className="ui fourteen wide column">
											...
											<div className="ui row">
												<div
													className="role"
													style={{ paddingLeft: "0rem", display: "inline" }}
												>
											...
											{this.state.part.etat === "REFUSE" && (
												<div className="ui row">
													<div
														className="ui sixteen wide column refuse"
									...
									<div className="ui row" style={{ padding: "1rem" }}>
										...
										>
											<img
												...
												alt=""
												className="ui spaced avatar image"
												src={`${config.IMAGE_SRV_URL}${this.state.donateur.avatarImage}`}
											/>
										</div>
										<div className="ui fourteen wide column">
											<div className="ui row">
												<div
													...
													style={{ display: "inline" }}
												>
													{this.state.donateur && this.state.donateur.artistName
														? this.state.donateur.artistName
														: `${this.state.donateur.firstName} ${this.state.donateur.lastName}`}
												</div>
												<div className="vote" style={{ float: "right" }}>
													{parseFloat(this.state.part.rightHolderPct).toFixed(
														2
													) + "%"}
												</div>
											</div>
											<div className="ui row">
												...
												>
													{t(
														"flot.split.documente-ton-oeuvre.editeur.donateur"
													)}
												</div>
												<div className="statut">
													<div className={"approuve"}>
														{t(`flot.split.vote.accept`)}
													</div>
												</div>
											</div>
										</div>
							...
							<div className="ui eight wide column">
								<div className="ui row">
									<div className="ui sixteen wide column">{visualisation}</div>
								</div>
							</div>
						</div>
					</div>
							...
							<div className="ui grid">
								<div className="ui row" style={{ marginBottom: "1rem" }}>
									...
									<div
										...
										style={{ textAlign: "right", paddingRight: "4.5rem" }}
									>
										{!this.estVoteFinal() && (
											<span
												style={{
