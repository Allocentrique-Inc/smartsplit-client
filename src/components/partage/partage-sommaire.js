...
class SommairePartage extends Component {
	...
	rafraichissementAutomatique() {
		setTimeout(() => {
			this.rafraichirDonnees(() => {
				if (
					(!this.estVoteFinal() && this.estVoteClos()) ||
					this.state.rafraichir
				) {
		...
		if (proposition) {
			...
			Object.keys(proposition.rightsSplits).forEach(famille => {
				Object.keys(proposition.rightsSplits[famille]).forEach(type => {
					proposition.rightsSplits[famille][type].forEach(partage => {
						if (
							partage.rightHolder.rightHolderId ===
								this.state.ayantDroit.rightHolderId &&
							partage.voteStatus === "active"
						) {
							termine = false
						}
					})
			...
			Object.keys(_droits).forEach(_type => {
				...
				_parts.forEach(_part => {
					if (_part.rightHolder) {
						if (
							_part.rightHolder.rightHolderId ===
							this.state.ayantDroit.rightHolderId
						) {
							if (!_mesVotes[_droit]) {
								_mesVotes[_droit] = {}
	...
	render() {
		...
		let t = this.props.t
		let nbVotesCumules = 0,
			nbVotesTotal = Object.keys(this.state.mesVotes).length
			...
			<div>
									...
									<div className="ui grid">
										<div className="ui row" style={{ marginBottom: "1rem" }}>
											...
											>
												...
												>
													{t("navigation.vote.requis", {
														nb: nbVotesCumules,
														total: nbVotesTotal,
														nombre: nbVotesTotal > 1 ? "s" : ""
													})}
												</span>
											</div>
				...
				{this.state.ayantDroit && (
					<Declaration
						...
						onClose={() => {
							this.setState({ modaleDeclaration: false })
						}}
						fn={() => {
							let body = {
							...
							axios
								.post(`${config.API_URL}proposal/voter`, body)
								.then(res => {
									utils.naviguerVersSommairePartage(
										this.state.proposition.mediaId
									)
								})
								.catch(err => {
									journal.error(err)
