...
export default class ArtistSelectDropdown extends React.PureComponent {
	...
	render() {
		...
		for(let key in this.state.artists) {
			...
			if(!compareArtists(artist, this.state.searchText))
				continue
			
			artists.push(<ArtistDropdownRow
				key={key}
				artistKey={key}
				value={artist}
		...
		const addNew = <TouchableWithoutFeedback onPress={this.addSearchAsNew}>
			<Layer padding="inside">
				<Text>Ajouter <Text bold>{this.state.searchText}</Text> comme nouvel artiste ou groupe</Text>
			</Layer>
